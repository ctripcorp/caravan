...
public final class ConcurrentHashMapValues {
    ...
    private static <K, V> void checkArgument(ConcurrentHashMap<K, V> map, K key, Func1<K, V> valueCreator) {
        NullArgumentChecker.DEFAULT.check(map, "map");
        NullArgumentChecker.DEFAULT.check(key, "key");
        NullArgumentChecker.DEFAULT.check(valueCreator, "valueCreator");
    }
    ...
    private static <K, V> V internalGetOrAdd(ConcurrentHashMap<K, V> map, K key, Func1<K, V> valueCreator) {
        V value = map.get(key);
        if (value != null)
            return value;

        value = valueCreator.execute(key);
        if (value == null)
            throw new IllegalArgumentException("valueCreator create a null value!");
    ...
    public static <K, V> V getOrAdd(ConcurrentHashMap<K, V> map, K key, Func1<K, V> valueCreator) {
        checkArgument(map, key, valueCreator);
        return internalGetOrAdd(map, key, valueCreator);
    }

    public static <K, V> V getOrAddWithLock(ConcurrentHashMap<K, V> map, K key, Func1<K, V> valueCreator) {
        checkArgument(map, key, valueCreator);
